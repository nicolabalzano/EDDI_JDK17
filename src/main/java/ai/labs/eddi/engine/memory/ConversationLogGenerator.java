    ...
    public ConversationLog generate(int logSize, boolean includeFirstBotMessage) {
        ...
        if (logSize != 0) {
            ...
            for (var index = startIndex; index < conversationOutputs.size(); index++) {
                ...
                var contentList = new LinkedList<Content>();                if (!isNullOrEmpty(context) &&
                        context.get(KEY_INPUT_FILES) instanceof List<?> inputFilesList &&
                        !inputFilesList.isEmpty() &&
                        inputFilesList.get(0) instanceof Map) {
                ...
                if (output instanceof List<?> outputList) {
                    if (!outputList.isEmpty()) {
                        ...
                        if (outputList.get(0) instanceof Map) {
                            @SuppressWarnings("unchecked")
                        ...
                        } else if (outputList.get(0) instanceof TextOutputItem) {
                            @SuppressWarnings("unchecked")
                ...
                conversationLog.getMessages().remove(0);
            }
        }
