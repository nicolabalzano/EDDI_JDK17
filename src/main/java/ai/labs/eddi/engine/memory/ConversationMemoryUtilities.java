...
public class ConversationMemoryUtilities {
    ...
    public static SimpleConversationMemorySnapshot convertSimpleConversationMemory(
        ...
        newSnapshot.getConversationProperties().putAll(conversationMemorySnapshot.getConversationProperties());        var conversationOutputs = conversationMemorySnapshot.getConversationOutputs();
        conversationOutputs = returnCurrentStepOnly ? List.of(conversationOutputs.get(conversationOutputs.size() - 1)) : conversationOutputs;
        if (returnDetailed) {
        ...
        conversationSteps = returnCurrentStepOnly ? List.of(conversationSteps.get(conversationSteps.size() - 1)) : conversationSteps;
    ...
    public static SimpleConversationMemorySnapshot convertSimpleConversationMemorySnapshot(
        ...
        if (returnCurrentStepOnly) {
            if (isNullOrEmpty(returningFields) || returningFields.contains(KEY_CONVERSATION_STEPS)) {
                var conversationSteps = memorySnapshot.getConversationSteps();                if (!conversationSteps.isEmpty()) {
            ...
            if (isNullOrEmpty(returningFields) || returningFields.contains(KEY_CONVERSATION_OUTPUTS)) {
                var conversationOutputs = memorySnapshot.getConversationOutputs();                if (!conversationOutputs.isEmpty()) {
